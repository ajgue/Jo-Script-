...
function MakeWindow(Configs)
  ...
  function AddDropdown(parent, Configs)
    ...
    local SelectedColor = Create("Frame", Frame, {
        BackgroundColor3 = Default,
        BorderSizePixel = 0,
        Position = UDim2.new(1, -20, 0.5, 0),
        AnchorPoint = Vector2.new(1, 0.5),
        Size = UDim2.new(0, 20, 0, 20)
    }) Corner(SelectedColor) Stroke(SelectedColor)
    ...
    for _, color in pairs(Options) do
        local ColorOption = Create("TextButton", ScrollBar, {
            Size = UDim2.new(1, 0, 0, 25),
            BackgroundColor3 = color,
            Text = "",
            AutoButtonColor = false
        }) Corner(ColorOption) Stroke(ColorOption)
